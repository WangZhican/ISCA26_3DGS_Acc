...
std::tuple<at::Tensor, at::Tensor, at::Tensor> intersect_tile(
    ...
    if (n_elements) {
        if(early_sorting){
            launch_only_encode_tile_kernel(
                // inputs
                means2d,
                radii,
                depths,
    ...
    at::Tensor isect_ids;
    if (early_sorting){
        isect_ids = at::empty({n_isects}, depths.options().dtype(at::kInt));
    }else{
        isect_ids = at::empty({n_isects}, depths.options().dtype(at::kLong));
    }
...
at::Tensor intersect_offset(
    ...
    if(early_sorting){
        launch_intersect_es_offset_kernel(
