...
template <typename scalar_t>
__global__ void only_encode_tile_kernel(
    // if the data is [C, N, ...] or [nnz, ...] (packed)
    const bool packed,
    // parallelize over C * N, only used if packed is False
    const uint32_t C,
    ...
    const scalar_t *__restrict__ means2d,            // [C, N, 2] or [nnz, 2]
    const int32_t *__restrict__ radii,               // [C, N] or [nnz]
    const int32_t *__restrict__ depths,             // [C, N] or [nnz]
    const int64_t *__restrict__ cum_tiles_per_gauss, // [C, N] or [nnz]
    const uint32_t tile_size,
    const uint32_t tile_width,
    const uint32_t tile_height,
    const uint32_t tile_n_bits,
    int32_t *__restrict__ tiles_per_gauss, // [C, N] or [nnz]
    int32_t *__restrict__ isect_ids,       // [n_isects]
    int32_t *__restrict__ flatten_ids      // [n_isects]
) {
    // parallelize over C * N.
    uint32_t id = cg::this_grid().thread_rank();
    uint32_t idx = depths[id];
    bool first_pass = cum_tiles_per_gauss == nullptr;
    if (id >= (packed ? nnz : C * N)) {
        return;
    }
    ...
    if (first_pass) {
        // first pass only writes out tiles_per_gauss
        tiles_per_gauss[id] = static_cast<int32_t>(
            (tile_max.y - tile_min.y) * (tile_max.x - tile_min.x)
        );
        return;
    }
    ...
    const int32_t cid_enc = static_cast<int32_t>(cid);

    int64_t cur_idx = (id == 0) ? 0 : cum_tiles_per_gauss[id - 1];
    for (int32_t i = tile_min.y; i < tile_max.y; ++i) {
        for (int32_t j = tile_min.x; j < tile_max.x; ++j) {
            int32_t tile_id = i * tile_width + j;
            // e.g. tile_n_bits = 22:
            // camera id (10 bits) | tile id (22 bits) | depth (32 bits)
...
void launch_only_encode_tile_kernel(
    // inputs
    const at::Tensor means2d,                    // [C, N, 2] or [nnz, 2]
    const at::Tensor radii,                      // [C, N] or [nnz]
    const at::Tensor depths,                     // [C, N] or [nnz]
    ...
    AT_DISPATCH_FLOATING_TYPES(
        means2d.scalar_type(),
        "only_encode_tile_kernel",
        [&]() {
            only_encode_tile_kernel<scalar_t>
                <<<grid,
                   threads,
                   shmem_size,
                   at::cuda::getCurrentCUDAStream()>>>(
                    ...
                    tiles_per_gauss.has_value()
                        ...
                        : nullptr,
                    isect_ids.has_value()
                        ? isect_ids.value().data_ptr<int32_t>()
                        : nullptr,
                    flatten_ids.has_value()
                        ? flatten_ids.value().data_ptr<int32_t>()
                        : nullptr
...
__global__ void intersect_es_offset_kernel(
    const uint32_t n_isects,
    const int32_t *__restrict__ isect_ids,
    const uint32_t C,
    const uint32_t n_tiles,
    const uint32_t tile_n_bits,
    int32_t *__restrict__ offsets // [C, n_tiles]
    ...
    int32_t isect_id_curr = isect_ids[idx];
    int32_t cid_curr = isect_id_curr >> tile_n_bits;
    int32_t tid_curr = isect_id_curr & ((1 << tile_n_bits) - 1);
    int32_t id_curr = cid_curr * n_tiles + tid_curr;
    ...
    if (idx > 0) {
        // visit the current and previous isect_id and check if the (cid,
        // tile_id) pair changes.
        int32_t isect_id_prev = isect_ids[idx - 1]; // shift out the depth
        if (isect_id_prev == isect_id_curr)
            return;

        // write out the offsets between the previous and current tiles
        int32_t cid_prev = isect_id_prev >> tile_n_bits;
        int32_t tid_prev = isect_id_prev & ((1 << tile_n_bits) - 1);
        int32_t id_prev = cid_prev * n_tiles + tid_prev;
        for (uint32_t i = id_prev + 1; i < id_curr + 1; ++i)
            offsets[i] = static_cast<int32_t>(idx);
    }
}

void launch_intersect_es_offset_kernel(
    // inputs
    const at::Tensor isect_ids, // [n_isects]
    const uint32_t C,
    const uint32_t tile_width,
    ...
    uint32_t n_tiles = tile_width * tile_height;
    uint32_t tile_n_bits = (uint32_t)floor(log2(n_tiles)) + 1;
    intersect_es_offset_kernel<<<
        grid,
        threads,
        shmem_size,
        at::cuda::getCurrentCUDAStream()>>>(
        n_elements,
        isect_ids.data_ptr<int32_t>(),
        C,
        n_tiles,
        tile_n_bits,
        offsets.data_ptr<int32_t>()
...
// https://nvidia.github.io/cccl/cub/api/structcub_1_1DeviceRadixSort.html
// DoubleBuffer reduce the auxiliary memory usage from O(N+P) to O(P)
void radix_tile_sort_double_buffer(
    const int64_t n_isects,
    const uint32_t tile_n_bits,
    const uint32_t cam_n_bits,
    at::Tensor isect_ids,
    ...
    cub::DoubleBuffer<int32_t> d_keys(
        isect_ids.data_ptr<int32_t>(), isect_ids_sorted.data_ptr<int32_t>()
    );
    cub::DoubleBuffer<int32_t> d_values(
        flatten_ids.data_ptr<int32_t>(), flatten_ids_sorted.data_ptr<int32_t>()
    );
